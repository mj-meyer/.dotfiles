...
// 1Password secret reference — change vault/item/field to match your setup
const EXA_API_KEY_OP_REF = "op://Private/Exa - API - Personal Default /credential";
...
function getExaApiKey(): string {
	...
	// Try env var first (fallback for CI or non-1Password setups)
	if (process.env.EXA_API_KEY) {
		cachedApiKey = process.env.EXA_API_KEY;
		return cachedApiKey;
	}

	try {
		const opPath = process.env.PATH?.includes("/opt/homebrew/bin") ? "op" : "/opt/homebrew/bin/op";
		cachedApiKey = execSync(`${opPath} read "${EXA_API_KEY_OP_REF}"`, {
			encoding: "utf8",
			timeout: 10_000,
		...
		throw new Error(
			"Failed to read Exa API key from 1Password. Ensure `op` CLI is installed, " +
			"CLI integration is enabled in 1Password, and the item exists at: " +
			EXA_API_KEY_OP_REF,
		);
	}
}
